...
public class RegExpTest extends RegExpUnary implements RegExpTemp {
    ...
    public QuotedFormula deltaDiamond(LDLfFormula goal, TransitionLabel label, LDLfFormula lastCall) {
        ...
        return new QuotedAndFormula(left.delta(label, lastCall), right.delta(label, lastCall));
    }
    ...
    public QuotedFormula deltaBox(LDLfFormula goal, TransitionLabel label, LDLfFormula lastCall) {
        ...
        return new QuotedOrFormula(left.delta(label, lastCall), right.delta(label, lastCall));
    }
}
